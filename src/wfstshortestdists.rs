...
use wfst::algorithms::shortestpath::shortest_distance;
...
fn wfstprint<W: Weight + Serialize, F: MutableFst<W> + ExpandedFst<W> + Display>(mut fst: F, isyms: Option<Vec<String>>, osyms: Option<Vec<String>>, isymfn: Option<String>, osymfn: Option<String>, mapsyms: bool) -> Result<(), IOError> {
    ...
    //Messy that shortest_distance() requires Serialize...
    let dists = shortest_distance(fst.clone()); //Clone is on ExpandedFst
    for (i, d) in dists.into_iter().enumerate() {
        println!("{}\t{:?}", i, d); //At some stage implement Display for weights
    }
    //Debugging info from here:
    eprintln!("");
    eprintln!("{}", fst);
    Ok(())
}
